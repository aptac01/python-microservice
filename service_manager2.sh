...
config_filename="./env_vars_w.ini"
...
Если флаги -c или -r не были переданы - вместо них будут использованы дефолтные значения. 
...
# нужно больше обфусцировать код, каждый раз показывать разную картинку и убрать упоминание того флага, который используется для активации
